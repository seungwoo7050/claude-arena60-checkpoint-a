...
class WebSocketServer::ClientSession : public std::enable_shared_from_this<WebSocketServer::ClientSession> {
 public:
 ...
 private:
  void DoEnqueueState(const PlayerState& state, std::uint64_t tick, double delta) {
    ...
    oss << "state " << state.player_id << ' ' << state.x << ' ' << state.y << ' ' << state.facing_radians
        << ' ' << tick << ' ' << delta;
    {
  ...
  void OnRead(std::size_t /*bytes_read*/) {
    ...
    if (player_id_.empty()) {
      ...
      session_.UpsertPlayer(player_id_);
    }
...
void WebSocketServer::Start() {
  ...
  loop_.SetUpdateCallback([self](const TickInfo& tick) {
    boost::asio::post(self->io_context_, [self, tick]() {
      self->BroadcastState(tick.tick, tick.delta_seconds);
    });
  });
  DoAccept();
}
...
std::string WebSocketServer::MetricsSnapshot() const {
  std::ostringstream oss;
  oss << "# TYPE websocket_last_tick counter\n";
  oss << "websocket_last_tick " << last_broadcast_tick_ << "\n";
  return oss.str();
}
